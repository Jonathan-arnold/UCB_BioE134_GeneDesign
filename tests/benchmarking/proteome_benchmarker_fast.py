...
def benchmark_proteome(fasta_file):
    ...
    # Randomly sample 10% of the proteome
    all_genes = list(proteome.keys())
    sample_size = max(1, int(len(all_genes) * 0.1))
    sampled_genes = random.sample(all_genes, sample_size)

    successful_results = []
    error_results = []

    for gene in tqdm(sampled_genes, desc="Processing genes"):
        protein = proteome[gene]
        try:
            ignores = set()
    ...
    for result in successful_results:
        ...
        if not codons_above_board:
            ...
            if codon_diversity < 0.5:
                sub_failures.append("Low Diversity")
            if rare_codon_count > 3:
                sub_failures.append("Excess Rare Codons")
            if cai_value < 0.2:
                sub_failures.append("Low CAI")
            validation_failures.append({
                'gene': result['gene'],
                'protein': result['protein'],
                'cds': cds,
                'site': f"Codon usage check failed [{', '.join(sub_failures)}]: Diversity={codon_diversity}, Rare Codons={rare_codon_count}, CAI={cai_value}"
            })
...
def generate_summary(total_genes, parsing_time, execution_time, errors_summary, validation_failures):
    ...
    for failure in validation_failures:
        ...
        if "Forbidden sequence" in site:
            ...
            for sub in codon_sub_failures:
                if sub in site:
                    codon_sub_failures[sub] += 1
    ...
    with open('summary_report.txt', 'w') as f:
        ...
        for checker, count in checker_failures.items():
            ...
            if checker == 'Codon Usage Checker' and count > 0:
                for sub, sub_count in codon_sub_failures.items():
                    f.write(f"    - {sub}: {sub_count} occurrences\n")
